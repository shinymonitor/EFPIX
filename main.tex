...
A \texttt{known} connection exists when the sender and receiver know each other’s public key and \texttt{alias}. \texttt{Alias} is a lookup key/reference for a public key. The sender includes their alias in messages, allowing the receiver to retrieve the sender’s public key for signature verification. If a message contains an unknown alias, it is treated as \texttt{anonymous}, and the signature is not verified. To prevent adversaries from linking messages to a common origin, users should practice good operational security by using unique aliases and key-pairs for each correspondent. Aliases are recommended to be unique random identifiers or truncated hashes of the public key to prevent collisions. The management of these identities is delegated to the user or higher-level applications, similar to identity management in protocols like Bitmessage. In practice, key exchange may itself be performed over EFPIX at a higher layer, using anonymous and unverified messages to transmit public keys and aliases as a "Connection Request".
...
\texttt{Sender Alias}: It serves as a key/reference to look up the sender’s public key to the receiver in its memory. It is agreed upon before the start of the communication. It can be different for different receivers. It is retrieved from the local "Contact" map (mapping (other alias) to {public Key, my alias}).\\
...
This concatenated data is used to generate a signature using the sender’s private key. 
Then the signature is also concatenated with the payload. 
This is encrypted using the receiver's public key which is also retrieved from the local Contact map. Finally, nonce and encrypted blob are concatenated to produce the final encoded message.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.7\linewidth]{encode.png}
...
However, the nonce is excluded from the deduplication hash calculation. This is done because an attacker can find a new valid nonce, resulting in a different deduplication hash even though the packet is the same.
The PoW difficulty must be calibrated such that the cost of generation exceeds the aggregate cost of trial decryption across the local mesh neighborhood.\\
...
Signature: The receiver verifies the message integrity and authenticity using the sender’s public key and signature. Inclusion of the timestamp in the signature serves as a simple counter to general replay attacks. For public key cryptography with payload size limit like RSA, the signature may be appended outside the encrypted blob, at the cost of lesser metadata protection by hiding it from network observers. 

\section{Decoding and Relaying}

\texttt{Timestamp of reception}: The reception timestamp is recorded.\\
\texttt{Deduplication}: The PoW is verified. Deduplication hash is calculated on the packet excluding the nonce. It is used to determine whether the relaying nodes have already seen this message. It prevents infinite looping relays. Every node keeps a record of hashes of previously relayed messages. If it has seen the hash, the message is discarded. If not, the hash is added to the record and the message is relayed to all neighbors.\\
\texttt{Decryption}: Then the node attempts decryption on the encrypted part. If decryption fails, the message is discarded. Otherwise, the original components are extracted (timestamp of creation, sender alias, internal address, and decoded message).\\
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.9\linewidth]{decode.png}
    \caption{Message decoding and relay process}
\end{figure}

\section{Optional Improvements}
\texttt{Sector-based routing}: To improve efficiency in large networks, the message can be flooded in only specific regions called \texttt{sectors}, which is an arbitrary group of nodes. However, this may come at the cost of a greater risk of deanonymization of the receiver and sender. Moreover, the sender must know which sector the receiver is in.\\
...
\texttt{Alternative Proof-of-Work Mechanisms}: Standard proof-of-work is vulnerable to adversaries with high-end parallel hardware (GPUs, ASICs). A better alternative is the use of Verifiable Delay Functions (VDFs) \cite{vdf2019}. VDFs require a specific duration of sequential computation, effectively neutralizing the advantage of parallel processors. This also provides more predictable message verification times.\\
...
This example uses Curve25519 (X25519 for key exchange, Ed25519 for signatures) \cite{curve255192006}, BLAKE2b for hashing \cite{blake22013}, and authenticated encryption with XChaCha20-Poly1305. The proof-of-work requires a configurable number of leading zeros in the BLAKE2b hash. For further security against quantum adversaries, post-quantum algorithms such as Kyber for encryption and Dilithium for signatures could be integrated. \\

Pre-encryption data format for unicast messages (348 bytes):
\begin{table}[h]
\centering
\begin{tabular}{|c|c|c|c|c|}
\hline
\textbf{Sender Alias} & \textbf{Timestamp} & \textbf{Internal Address} & \textbf{Message} & \textbf{Signature} \\
\hline
16 bytes & 8 bytes & 4 bytes & 256 bytes & 64 bytes \\
\hline
\end{tabular}
\end{table}

Encoded unicast message layout (430 bytes):
\begin{table}[h]
\centering
\begin{tabular}{|c|c|c|c|c|c|c|}
\hline
\textbf{Version} & \textbf{Type} & \textbf{MAC} & \textbf{Nonce} & \textbf{Ephemeral PK} & \textbf{Ciphertext} & \textbf{PoW Nonce} \\
\hline
1 byte & 1 byte & 16 bytes & 24 bytes & 32 bytes & 348 bytes & 8 bytes \\
\hline
\end{tabular}
\end{table}

Broadcast message layout (signed, 430 bytes):
\begin{table}[h]
\centering
\begin{tabular}{|c|c|c|c|c|c|c|c|}
\hline
\textbf{Version} & \textbf{Type} & \textbf{Sign PK} & \textbf{Timestamp} & \textbf{Int. Addr.} & \textbf{Message} & \textbf{Signature} & \textbf{PoW Nonce} \\
\hline
1 byte & 1 byte & 32 bytes & 8 bytes & 4 bytes & 312 bytes & 64 bytes & 8 bytes \\
\hline
\end{tabular}
\end{table}
...
\section{Future Work}
Future work could explore integration with radio mesh hardware and secure bootstrap methods for alias public key exchanges. Implementations can simulate or deploy on different networks to provide performance and other results to compare against existing protocols that aim to solve a similar problem. 
...
\bibitem{curve255192006}
Daniel J. Bernstein, \textit{Curve25519: new Diffie-Hellman speed records},  
\url{https://cr.yp.to/ecdh/curve25519-20060209.pdf}, 2005.

\bibitem{blake22013}
Jean-Philippe Aumasson, Samuel Neves, Zooko Wilcox-O’Hearn, and Christian Winnerlein, \textit{BLAKE2: simpler, smaller, fast as MD5},  
\url{https://www.blake2.net/blake2.pdf}, 2013.
